...
# 5. Qual a quantidade de servidores cujo cargo básico é Analista de Políticas Públicas Gestão Governamental?
qtdd = dados["CARGO_BASICO"].str.contains("ANALISTA POLITICAS PUBLICAS GESTAO GOVERNAMENTAL", case=False).sum()
print(qtdd)


# 6. Qual o percentual de servidores do sexo feminino na Prefeitura?
mulheres = dados["SEXO"].str.contains("FEMININO", case=False).sum()
print(f"{(mulheres/total_serv):.2%}")


# 7. Qual o percentual de servidores cuja escolaridade do cargo básico é Superior Completo?**"""
